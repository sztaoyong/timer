...
function aht(){
    ...
    function incrementTimer(){
        if (active && localStorage.getItem(caseLocked)) { // increment if window is on focus and case is locked;
            timer++;
        }
    ...
    function computeAHT(){
        if (!supportsLocalStorage()) { 
            ...
            if(!localStorage.getItem(caseLocked)) {
            ...
            if (updateAHT(associateId, caseHandleTime)) { 
				...
				localStorage.removeItem(caseLocked);
                timer = 0;
                return true;
            }else{
                return false;
